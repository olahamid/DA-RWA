...
interface IOARWARegistry {

    function s_OAAssetAdmins() external view returns(bytes32);

    function s_OAAssetPlatformAdmins() external view returns(bytes32);

    function assetRegistration(OALibrary.Asset memory _asset, address _assetContractAddr, uint16 assetID ) external payable returns(bytes memory);
    ...
    // View Functions
    function getPlatform(bytes memory platformBytes) external view returns (OALibrary.Platform memory);
    function getAsset(address platformCreator, uint16 platformId) external view returns (OALibrary.Asset memory);
